...
static int ac107_probe(struct snd_soc_codec *codec)
{
	...
	codec->control_data = devm_regmap_init_i2c(ac107->i2c, &ac107_regmap_config);
	...
	snd_soc_add_codec_controls(codec, ac107_volume_controls, ARRAY_SIZE(ac107_volume_controls));	//PGA Gain Control & Digital Volume Control
...
static unsigned int ac107_codec_read(struct snd_soc_codec *codec, unsigned int reg)
{
	//AC107_DEBUG("\n--->%s\n",__FUNCTION__);
	u8 val_r;
	struct ac107_priv *ac107 = dev_get_drvdata(codec->dev);
...
static int ac107_codec_write(struct snd_soc_codec *codec, unsigned int reg, unsigned int value)
{
	//AC107_DEBUG("\n--->%s\n",__FUNCTION__);
...
#ifdef AC107_EXT_VDD
	struct pinctrl *pinctrl_ac107;
	struct pinctrl_state *power_on,*power_off;
#endif

	int ret = 0;

	printk("ac107_i2c_probe start\n");
...
#ifdef AC107_EXT_VDD
		pinctrl_ac107 = devm_pinctrl_get(&i2c->dev);
			if (IS_ERR(pinctrl_ac107)) {
				ret = PTR_ERR(pinctrl_ac107);
				printk("%s,Cannot find pinctrl2!\n",__func__);
				return ret;
			}
			power_off = pinctrl_lookup_state(pinctrl_ac107, "state_power_off");
			if (IS_ERR(power_off)) {
				ret = PTR_ERR(power_off);
				printk( "%s,Cannot find touch pinctrl state_power_off!\n",__func__);
				return ret;
			}
			
			power_on = pinctrl_lookup_state(pinctrl_ac107, "state_power_on");
			if (IS_ERR(power_on)) {
					ret = PTR_ERR(power_on);
					printk( "%s,Cannot find touch pinctrl state_power_on!\n",__func__);
					return ret;
			}
			pinctrl_select_state(pinctrl_ac107, power_on);
